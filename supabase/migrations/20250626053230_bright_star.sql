...
DROP POLICY IF EXISTS "Creators can insert their own posts" ON feed_posts;
DROP POLICY IF EXISTS "Creators can update their own posts" ON feed_posts;
DROP POLICY IF EXISTS "Creators can delete their own posts" ON feed_posts;
...
DROP POLICY IF EXISTS "Anyone can view star donations" ON star_donations;
DROP POLICY IF EXISTS "Users can send star donations" ON star_donations;
...
-- Insert sample data only if creators table is empty
DO $$
DECLARE
...
BEGIN
  ...
  -- Only proceed if no creators exist
  IF creator_count = 0 THEN
    -- Get a sample user ID from existing users table (if any exists)
    SELECT id INTO sample_user_id FROM users LIMIT 1;
    
    -- Generate creator IDs
    creator1_id := gen_random_uuid();
    creator2_id := gen_random_uuid();
    creator3_id := gen_random_uuid();
    creator4_id := gen_random_uuid();
    creator5_id := gen_random_uuid();
